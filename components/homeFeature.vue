...
const loadCategories = async () => {
  try {
    ...
    if (result.categories && result.categories.length > 0) {
      ...
      // If we have less than 8 categories, add some fallback ones to fill
      if (categories.value.length < 8) {
        const additionalCategories = fallbackCategories
          .slice(0, 8 - categories.value.length)
          .map(item => ({
            id: item.id + 100, // Add offset to avoid ID conflicts
            name: item.title,
            image: item.image,
        ...
        categories.value = [...categories.value, ...additionalCategories]
      }
